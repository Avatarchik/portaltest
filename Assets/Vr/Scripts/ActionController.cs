...
{
    public GameObject TeleporterHand;
    public GameObject TeleportCursor;

    public GameObject SelectedGameObject;
    public Vector3 CursorPoisition;
    ...
    void Start()
    {
        var trackedController = TeleporterHand.GetComponent<SteamVR_TrackedController>();
        if (trackedController == null)
        {
            trackedController = TeleporterHand.AddComponent<SteamVR_TrackedController>();
        }
    ...
    private void Update()
    {
        transform.position = TeleporterHand.transform.position;
        transform.rotation = TeleporterHand.transform.rotation;

        RaycastHit raycastHitInfo;
        var ray2 = new Ray(transform.position, transform.forward);
        var hasHit = Physics.Raycast(ray2, out raycastHitInfo);

        if (hasHit)
        {
            TeleportCursor.transform.position = raycastHitInfo.point;
            SelectedGameObject = raycastHitInfo.collider.gameObject;
        }
        else
        {
        ...
        {
            ChangeCursorColor(Action.Teleport);
            var ca = SelectedGameObject.GetComponent<IClickAction>();
            
            if (ca != null)
            {
                ChangeCursorColor(Action.PressButton);
            }
        } else
        {
            ChangeCursorColor(Action.Empty);
        }

    }

    void ChangeCursorColor(Action actionType)
    {
        var ren = TeleportCursor.GetComponent<Renderer>();
    ...
    {
        var TeleportNow = teleportOnClick;
